...
Call `get_sick_leave_guidance`. Use only its returned sections.
...
local rules. If `ask_for_location` is true, the country picker was already
sent; on their reply call again with `location_override`.
