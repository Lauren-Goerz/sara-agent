...
async def get_days_until_payday(
    ...
    if geo is None:
        return ToolResult(
            llm_response={
                "ok": False,
                "error": "location_needed",
                "ask_for_location": True,
                "timezone": timezone,
                "profile_location": profile_location,
                "message": (
                    "Need employment location or pay setup: Germany, Serbia, "
                    "France, UK (around the 26th); US (twice a month); or Deel "
                    "(twice a month or last day of month)."
                ),
                "instruction": (
                    "Ask one short clarifying question for their employment "
                    "country / whether they are on Deel. Do not invent a "
                    "payday."
                ),
            }
        )
