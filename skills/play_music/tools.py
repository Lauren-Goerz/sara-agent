...
async def _search_track(query: str) -> dict[str, Any] | None:
    ...
    if _spotify_configured():
        try:
            ...
            if hit:
                return hit
            structlogger.info("play_music.spotify_no_results", query=query)
        except Exception as error:  # noqa: BLE001
            structlogger.warning(
                "play_music.spotify_failed_falling_back",
                error=str(error),
                query=query,
            )
...
async def send_slack_song(
    ...
    try:
        ...
        structlogger.error("play_music.search_failed", error=str(error), query=search)
        return ToolResult(
            llm_response={
                "ok": False,
                "error": "search_failed",
